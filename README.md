...
> A browser-based Stable Diffusion stress test and benchmark tool for AMD Ryzen AI NPU (XDNA / XDNA2).  
> Fills the gap left by Procyon AI — which does not support NPU-accelerated image generation testing.
...
Most AI benchmark tools (including Procyon AI) test image generation on CPU or GPU only. This tool runs Stable Diffusion **natively on the AMD Ryzen AI NPU** via the VitisAI execution provider, giving you real NPU utilization data that no other benchmark currently provides.
...
- AMD Ryzen AI processor with XDNA or XDNA2 NPU
...
[Procyon AI Benchmark](https://procyon.ai) is a popular tool for benchmarking AI workloads on consumer hardware. However, as of 2026, it does not support NPU-accelerated Stable Diffusion testing — all its image generation tests run on CPU or GPU.

This tool was built to fill that gap, providing the first open benchmark specifically targeting the AMD XDNA2 NPU for image generation workloads.
...
*Design by Hiko — built on AMD Ryzen AI XDNA2 hardware*
